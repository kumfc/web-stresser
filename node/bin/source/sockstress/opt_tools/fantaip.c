...
static int broadcast_arp(uint16_t type, uint32_t addr) {
	...
	if (buf_size < 1 || pbuf == NULL) {
		MSG(M_ERR, "makepkt fails, exiting");
		exit(1);
	}

	if (eth_send(bob.e, pbuf, buf_size) < 1) {
		MSG(M_ERR, "eth_send fails, exiting");
		exit(1);
	}
...
static int send_arp(struct myetheraddr *dst, uint32_t dstip) {
	...
	if (verbose > 1) {
		MSG(M_VERB, "Sending ARP resp to: %s", decode_6mac((const uint8_t *)&dst->octet[0]));
	}
...
int main(int argc, char ** argv) {
	...
	if (optind < argc) {
		...
		if (inet_aton(myip, &ia) == 0) {
			MSG(M_ERR, "Illegal IP address `%s'", myip);
			exit(1);
		}
	...
	if (verbose > 2) {
		if (bob.saddr_mask != 0xffffffff) {
			...
			MSG(M_VERB, "Using addresses `%s->%s/%u' (Netmask %u.%u.%u.%u)", myip, highip, bob.cidr, *(p), *(p + 1), *(p + 2), *(p + 3));
		}
		else {
			MSG(M_VERB, "Using address `%s'", myip);
		}
	}

	if (verbose && detach) {
		MSG(M_ERR, "Make up your mind, detach from the tty, or be chatty... not both");
		exit(1);
	}

	if (myip == NULL) {
		MSG(M_ERR, "IP address is required");
		usage();
	}

	if (bob.device == NULL) {
		MSG(M_ERR, "Interface argument is required");
		exit(1);
	}
	...
	if (bob.e == NULL) {
		MSG(M_ERR, "Can't open Ethernet link: %s", strerror(errno));
		exit(1);
	}
	...
	if (hwaddr != NULL) {
		...
		if (sscanf(hwaddr, "%x:%x:%x:%x:%x:%x", &hws[0], &hws[1], &hws[2], &hws[3], &hws[4], &hws[5]) != 6) {
			MSG(M_ERR, "Bad hardware address, use XX:XX:XX:XX:XX:XX, not `%s'", hwaddr);
			exit(1);
		}
		if (hws[0] > 255 || hws[1] > 255 || hws[2] > 255 || hws[3] > 255 || hws[4] > 255 || hws[5] > 255) {
			MSG(M_ERR, "No, thats not really going to work, sorry");
			exit(1);
		}
		...
		if (verbose) {
			MSG(M_DBG2, "Using hardware address %x:%x:%x:%x:%x:%x", hwaddrs[0], hwaddrs[1], hwaddrs[2], hwaddrs[3], hwaddrs[4], hwaddrs[5]);
		}
		memcpy((void *)&bob.shwaddr, (void *)&hwaddrs[0], 6);
	}

	else if (eth_get(bob.e, (eth_addr_t *)&bob.shwaddr) < 0) {
		MSG(M_ERR, "Can't get hardware address: %s", strerror(errno));
		exit(1);
	}
	...
	memset(errors, 0, sizeof(errors));
	pdev=pcap_open_live(bob.device, 500, 0, 0, errors);

	if (pdev == NULL) {
		MSG(M_ERR, "Can't open up interface `%s': %s", bob.device, errors);
		exit(1);
	}

	if (util_getheadersize(pdev, errors) != 14) {
		MSG(M_ERR, "You SURE this is an ethernet interface? doesnt look like one");
		exit(1);
	}

	if (util_preparepcap(pdev, errors) < 0) {
		MSG(M_ERR, "Can't prepare bpf socket: %s", strerror(errno));
		exit(1);
	}
	...
	if (pcap_setnonblock(pdev, 1, errors) < 0) {
		MSG(M_ERR, "Can't set pcap dev nonblocking: %s", errors);
		exit(1);
	}

	/*
	 * user override check for arp? 
	 */
	do {
		for (bob.addr_cleared=0, tries=0; bob.addr_cleared == 0 && tries < 3; tries++) {
			if (verbose > 3) {
				MSG(M_DBG2, "testing `%s'", inet_ntoa(*((struct in_addr *)&bob.saddr)));
			}
			...
			while (1) {
				...
				if (breakloop || bob.addr_cleared != 0) {
					break;
				}
			}
			alarm(0);
		...
		if (bob.addr_cleared == -1) {
			MSG(M_ERR, "Error: Address already in use");
			exit(1);
		}
		bob.saddr += htonl(1);
		if (1) {
	...
	if (verbose) {
		MSG(M_OUT, "Arping for %s/%u [%s]", inet_ntoa(*((const struct in_addr *)&bob.saddr)), bob.cidr, decode_6mac((const uint8_t *)&bob.shwaddr.octet[0]));
	}
...
void process_packet(uint8_t *user, const struct pcap_pkthdr *phdr, const uint8_t *packet) {
	...
	if (phdr->caplen < (sizeof(struct ether_header) + sizeof(struct arp_packet))) {
		MSG(M_ERR, "Short packet!!!!");
		return;
	}

	if (verbose > 4) {
		MSG(M_DBG2, "got packet hw type %u proto %x hwsize %x protosize %x", ntohs(ap->hw_type), ntohs(ap->protocol), ap->hwsize, ap->protosize);
	}
...
void do_daemon(void) {
	...
	child=fork();
	if (child < 0) {
		MSG(M_ERR, "Can't fork: %s", strerror(errno));
		exit(1);
	}
	else if (child == 0) {
		setsid();
