...
int rbinsert(void *lh, uint64_t key, void *data) {
	...
	DBG("rbinsert(lh, %llu, data)", key);
	if ((ret=_rb_insert(h_u.lh, key, &added)) < 0) {
		switch (ret) {
			case -1:
				DBG("cant insert key %llu into list, duplicate key", key);
				return -1;
			default:
				DBG("cant insert key %llu into list, unknown error `%d'", key, ret);
				return -1;
		}
	}
	...
	if (DEBUG) {
		...
		d_u.ptr=added->data;
		DBG("set node key %llu data to `%s'", key, d_u.str);
	}
...
int _rb_find(rhead *h, uint64_t key, rnode **node) {
	...
	DBG("in _rb_find(lh, %llu, node)", key);

	if (h->root == NULL) {
		*node=NULL;
		DBG("_rb_find() returning -1");
		return -1;
	}
	...
	for (walk=h->root ; walk != NULL ; ) {
		...
		if (key > walk->key) {
			DBG("_rfind Steping to right");
			walk=walk->rchld;
		}
		else {
			DBG("_rfind Steping to left");
			walk=walk->lchld;
		}
	}
...
static int _rb_insert(rhead *h, uint64_t key, rnode **result) {
	...
	DBG("in _rb_insert(h, %llu, result)", key);
	...
	if (h->root == NULL) {
		...
		h->root->key=key;
		h->len=1;
		DBG("Adding root node at %p with key %llu [%llu]", h->root, key, h->root->key);
		*result=h->root;
		return 1;
	}
	...
	if (parent->key > add->key) {
		DBG("Adding new node key %llu at %p to LEFT of parent key %llu at %p", add->key, add, parent->key, parent);
		parent->lchld=add;
	}
	else {
		DBG("Adding new node key %llu at %p to RIGHT of parent key %llu at %p", add->key, add, parent->key, parent);
		parent->rchld=add;
	}
	add->parent=parent;
	DBG("Parent %p\n", (void *)parent);
	...
	if (add->parent == h->root) {
		...
		*result=add;
		DBG("new node is already red and root node is already black, nothing to color");
		return 1;
	}
...
*/
	...
	while (1) {
		assert(node != NULL);
		if (node == h->root) {
			DBG("Breaking cause node is root node");
			break;
		}
		if (node->parent->color == black_e) {
			DBG("breaking cause node parent color is black");
			break;
		}
		/* we have broken rule number 3, lets fix it */
		...
		if (grandparent == h->root) {
			DBG("our grandparent is the root node");
		}

		DBG("COLORS: us %s [%llu] parent %s [%llu] grandparent %s [%llu] ", (node->color == black_e ? "black" : "red"),
		node->key, (parent->color == black_e ? "black" : "red"), parent->key,
		(grandparent->color == black_e ? "black" : "red"), grandparent->key);

		/* if our parent is the left child of our grandparent */
		if (parent == grandparent->lchld) {
			rnode *bob=NULL;

			DBG("our parent is the left child of the grandparent");
			/* bob is our uncle, but we never call him... */
			bob=grandparent->rchld;

			if (bob && bob->color == red_e) {
				DBG("setting parent to black far uncle to black and grandparent to red and becoming grandparent");
				...
				DBG("setting parent to black and grandparent to red and rotating grandparent");
				parent->color=black_e;
				grandparent->color=red_e;
			...
			DBG("our parent is the right child of the grandparent");
			/* bob is our uncle, but we never call him... */
	...
	if (h->root->color == red_e) {
		DBG("recoloring root node to black");
		h->root->color=black_e;
	}
	h->root->parent=NULL;
...
static void _rb_murder(rhead *h, rnode **n) {
	...
	FREE((*n));
	*n=NULL; /* this is to prevent mistakes ;] */
...
static int _rb_snuffout(rhead *h, rnode *n) {
	...
	if ((ret=_rb_find(h, n->key, &del)) < 0) {
		DBG("Can't find key %llu, status %d", n->key, ret);
		return -1;
	}
...
static rnode *_rb_find_successor(rnode *n) {
	...
	DBG("in _rb_find_successor(node->key = %llu)", n->key);
...
void rbdumptree(void *lh, rnode *n) {
	...
	if (n == NULL) {
		if (h_u.lh->root == NULL) return;
		n=h_u.lh->root;
	}

	printf("Node key %llx is %s\n", (long long unsigned)n->key, (n->color == black_e ? "Black" : "Red"));

	if (n->rchld != NULL) rbdumptree(lh, n->rchld);
	if (n->lchld != NULL) rbdumptree(lh, n->lchld);
